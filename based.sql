...
CREATE TABLE `organization` (
  ...
  `Budget_Equity` int(9) DEFAULT NULL CHECK (`Budget_Private_Actions` >= 0),
  PRIMARY KEY (`ID_Org`)
...
CREATE TABLE `project` (
  ...
  `Finish_Date` date NOT NULL CHECK (DATEDIFF(`Finish_Date`, `Start_Date`) > 0),
  `Funding_Amount` int(11) NOT NULL CHECK (`Funding_Amount` >= 100000 AND `Funding_Amount` <=1000000),
  `Duration` int AS (YEAR(`Finish_Date`) - YEAR(`Start_Date`)) CHECK (`Duration` <= 4),
  `ID_org` int(11) NOT NULL,
  `ID_Researcher_in_Charge` int(11) NOT NULL,
  `ID_Evaluator` int(11) NOT NULL,
  `Evaluation_Grade` int(11) NOT NULL CHECK (`Evaluation_Grade` >= 0),
  ...
  CONSTRAINT `ID_Program_Project` FOREIGN KEY (`ID_Program`) REFERENCES `program` (`ID_Program`) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT `ID_Executive_Project` FOREIGN KEY (`ID_Executive`) REFERENCES `executive` (`ID_Executive`) ON DELETE RESTRICT ON UPDATE CASCADE
  )ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
for each row
begin
		if (new.ID_Researcher_in_charge not in (select ID_Researcher from works where ID_Project = new.ID_Project)) then
        INSERT INTO works (`ID_Project`, `ID_Researcher`) values (new.ID_Project, new.ID_Researcher_in_Charge);
        end if;
        end;
        $$
...
for each row
begin
	if (old.ID_Researcher = (select ID_Researcher_in_Charge from project where ID_Project = old.ID_Project)) then
	signal sqlstate '45000' SET MESSAGE_TEXT = 'A chief researcher cannot be deleted. To do so define a new chief first.';
    end if;
    end;
$$
DELIMITER ;
